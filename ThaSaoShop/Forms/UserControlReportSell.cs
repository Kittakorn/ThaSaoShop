...
{
    public partial class UserControlReportSell : UserControl
    {
        public StoreEntities db;
        List<ViewSellOrBuy> viewSells;
        public UserControlReportSell(StoreEntities db)
        {
            this.db = db;
            InitializeComponent();
        }
        ...
        {
            viewSells = db.Database.SqlQuery<ViewSellOrBuy>("SELECT Date,Total FROM ViewSell").OrderBy(x => x.Date).ToList();
        ...
        {
            using (FormChartSell form = new FormChartSell(viewSells, false))
            {
                form.ShowDialog();
            }
        }
